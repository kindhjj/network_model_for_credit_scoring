...
class My_Table_Plotter():
    ...
    def get_coef_df(self):
        ...
        col_name_list = list()
        _ = list(map(lambda x: col_name_list.append(
            "V"+str(x+1)), range(len(m.coef_))))

        coef_df = pd.DataFrame(index=self.__keys, data=0,
    ...
    def get_num_of_selected_var_df(self, threshold=0.1, coef_threshold=0):
        num_of_selected_var_df = pd.DataFrame(
            index=self.__connect_name_list, columns=self.__model_name_list, data=np.nan)

        for model_name in self.__model_name_list:
            for connect_name in self.__connect_name_list:
                model = self.__model_dict[(
                    model_name, connect_name, threshold)]
                temp = model.get_best_model().coef_.copy()
                temp[temp >= coef_threshold] = 1
                temp[temp <= -coef_threshold] = 1
                temp[temp != 1] = 0
                num_of_selected_var_df.loc[connect_name,
                                           model_name] = np.sum(temp)
        ...
        for model_name in self.__model_name_list:
            ...
            for connect_name in self.__connect_name_list[:1]:
                ...
                false_positive_rate, true_positive_rate, thresholds = roc_curve(
                    cur_model.get_y_test(), cur_model.get_y_pred())
                roc_auc = auc(false_positive_rate, true_positive_rate)

                plt.plot(false_positive_rate, true_positive_rate,
                         label='%s = %0.4f' % (connect_name, roc_auc))
    ...
    def get_roc_df(self):
        ...
        for model_name in self.__model_name_list:
            ...
            roc_df = pd.concat([roc_df, pd.DataFrame(
                columns=[model_name], data=col_values)], axis=1)
